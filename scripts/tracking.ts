...
`;
  ...
  async updateWeeklyReview(weekNumber: number): Promise<void> {
    ...
    if (content.includes(`### Week: ${weekNumber}`)) {
      console.log(`Entry for week ${weekNumber} already exists. Skipping...`);
      return;
    }
...
`;
  ...
  async updateMonthlyReview(month: string, year: number): Promise<void> {
    ...
    const content = await fs.readFile(this.trackingFile, 'utf-8');
    if (content.includes(`### Month: ${month} ${year}`)) {
      console.log(`Entry for ${month} ${year} already exists. Skipping...`);
      return;
    }
